...
(tail call optimization), higher order functions and pattern matching;
moreover,  true functional languages are based on immutable data structures.
Scheme is somewhat less functional than SML and Haskell, since Scheme lists are mutable,
...
the next ones we will show many other functional idioms.
...
I should notice that the requirement of importing ``(rnrs mutable-pairs)``
is only valid for scripts and libraries: the behavior of the REPL is unspecified in
...
Excluding code typed at the REPL, in principle the compiler could use immutability
optimizations for library code not importing the ``(rnrs mutable-pairs)``
extension.  In practice, a Scheme compiler cannot perform the
optimizations based on the assumption of truly immutable pairs,
because the current standard says that ``cons`` must allocate a new
pair and cannot re-use a previously created one
...
since there is a compatibility breaking problem. I can only
...
default arguments) especially in the hairy situations of
multithreaded code.

In my *Adventures* I will never rely on the ability to mutate pairs,
and I will use pairs as functional data structures.
...
Notice that ``set-element`` is nicer that its imperative counterpart::
...
Notice the use of the R6RS procedure ``(list-tail lst n)`` which returns
the tail of ``lst`` starting from the n-th element (for instance
``(list-tail '(a b c d) 2)`` is the list ``(c d)``): the important
bit is that ``list-tail`` returns the tail, and *not a copy* of it,
so by mutating the tail you are actually mutating the original
list. That is clearly quite risky.

The R6RS standard does not provide primitives for functional update
out of the box, however it does provide primitive to remove elements for a list
functionally::
...
than C compilers in average, but they are not so bad. In most situations
the slowdown is acceptable and in particular situations compilers for
functional languages can be faster than a C compiler.
Moreover, they allow for memory optimizations. For instance, you can
memoize an immutable list, whereas
you cannot memoize a mutable one. Internally, a compiler is allowed to
memoize immutable data structures at will, so that using immutable data
structures does not necessarily means a large memory consumption.

Since I come from a Python background I do not care much about performance and
optimizations, but I care a lot about maintainability of programs and bug reduction,
and that is why I will use the functional paradigm whenever possible in this series.
