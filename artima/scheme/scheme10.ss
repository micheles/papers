...
``#<syntax >`` things. How do we get an error message which is
less scary to the newbie?

By using the guarded patterns feature of ``sweet-macros``.
Here is an example of how to use a guard:
...
as a preprocessor, as we planned to do.

Moreover, using pattern matching is much more readable than using
quasiquotation and the splice syntax, especially if you take
full advantage of the features of ``syntax-match``.
In particular, even if I did not have the time to mention it in the
previous episode, ``syntax-match`` allows for literal
...
used to enhance readability, or to define complex macros.
For instance our ``for`` macro can be written without need for
guarded patterns as::
...
(import (rnrs) (ikarus) (sweet-macros) (repeat-macro))
